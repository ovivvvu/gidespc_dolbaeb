...
# Ура
# код собран
# Мне кажется щас бота запустим нас перекинет на порно хаб . ru
# АХАХХАХАХАХАХ МНЕ ТОЖЕ ТАК КАЖЕТСЯ
# ты готов?
...
# добро пожаловать в УЖАСНЫЙ КРИНЖ КОД Finonclient что бы получить код мы ебали создателя 1год
# кста я рил его акк взломал
# я у него токен попросил
# потом пароль поменял)
# код готов в к запуску)
...
@client.slash_command(description="т фото животных.")
async def warn(ctx, member:discord.Member, *, reasonn="Без причины"):
    reason = reasonn + f"({ctx.message.author.name})"
    if member.mention == ctx.author.mention:
        await ctx.send("Нельзя так сделать на себе")
        return
    role = ctx.author.roles[len(ctx.author.roles)-1]
    merole = member.roles[len(member.roles)-1]
    if role.position <= merole.position:
        await ctx.send("Невозможно сделать это на данном участнике, потому что он вас выше")
        return
    if not os.path.exists(f"{ctx.guild.id}/{member.id}"):
        os.mkdir(f"{ctx.guild.id}/{member.id}")
    if not os.path.exists(f"{ctx.guild.id}/{member.id}/warns.txt"):
        open(f"{ctx.guild.id}/{member.id}/warns.txt", "w", encoding="utf8").write("")
    old = open(f"{ctx.guild.id}/{member.id}/warns.txt", "r", encoding="utf8").readlines()
    print(old)
    old.append(f"{reason}")
    open(f"{ctx.guild.id}/{member.id}/warns.txt", "w", encoding="utf8").write("\n".join(old))
    await ctx.send("Участник был предупреждён\nПричина: **{reason}**\nМодератор:{ctx.author.mention}")
    await ctx.send(member, "Участник был предупреждён", f"Вы были предупреждены на сервере **{ctx.guild.name}**\nПричина: **{reason}**\nМодератор:{ctx.author.name}")
